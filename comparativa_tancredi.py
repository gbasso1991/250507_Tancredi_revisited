...
for idc in idcs:
    ...
    titulo=f'{H_max:.1f} kA/m - {frec:.0f} kHz'


    fig, ax = plt.subplots(nrows=1,figsize=(6,5),constrained_layout=True)
    ax.plot(H_citrato/1000,M_citrato,label='ex citrato')
    ax.plot(H_hierro/1000,M_hierro,label='ex hierro')
    ax.plot(H_insitu/1000,M_insitu,label='ex in situ')
    ...
    # ax.set_xlim(0,60e3)
    # ax.set_ylim(0,)
    plt.savefig('comparativa_HM_tancredi'+str(idc)+'.png',dpi=400)
    plt.show()
